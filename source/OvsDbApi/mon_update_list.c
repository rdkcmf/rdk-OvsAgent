...
//static OVS_STATUS remove_msg_list(const char* UUID);
...
OVS_STATUS mon_list_remove(const char * uuid)
{
    ...
    if(uuid == NULL || msg_list == NULL)
    {
        OvsDbApiError("Trying to remove monitor node with either list empty or uuid NULL.\n");
        return OVS_FAILED_STATUS;
    }
...
OVS_STATUS mon_list_clear()
{
    ...
    OvsDbApiDebug("Clearing the monitor list.\n");
