...
#define GTEST_REPORT_FILEPATH      "/tmp/Gtest_Report/OvsAgent_gtest_report.xml"
#define GTEST_REPORT_FILEPATH_SIZE 128

class OvsAgentEnvironment : public ::testing::Environment
{
    public:
        virtual ~OvsAgentEnvironment()
        {
        }
        virtual void SetUp()
        {
            printf("%s Environment\n", __func__);
            if (open_log(OVS_AGENT_API_LOG_FILE, "OvsAgentTest"))
            {
...
GTEST_API_ int main(int argc, char *argv[])
{
    ...
    ::testing::AddGlobalTestEnvironment(new OvsAgentEnvironment());
