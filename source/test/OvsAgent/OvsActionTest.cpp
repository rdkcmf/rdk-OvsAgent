...
// RDKB-35546
TEST_F(OvsActionTestFixture, ovs_action_setup_offloading_in_router_mode_tchxb6)
{
    const OVS_IF_TYPE ifType = OVS_BRIDGE_IF_TYPE;
    const OVS_CMD     ifCmd = OVS_IF_UP_CMD;
    const std::string bridge = "br403";
    const std::string associatedBridge = "brlan0";
    const std::string mtu = "1500";
    const char expectedIP[] = "100.0.0.1";
    const char expectedCIDR[] = "100.0.0.0/24";
    char expectedModel[] = "CGM4140COM";
    ...
    expectedCmds.push_back("ifconfig " + bridge + " up");
    expectedCmds.push_back("ifconfig " + bridge + " mtu " + mtu);
    expectedCmds.push_back("ovs-ofctl --strict del-flows " + associatedBridge +
        " \"table=0, priority=50, ip_dst=" + expectedCIDR + ", ct_state=-trk, tcp\"");
    expectedCmds.push_back("ovs-ofctl --strict del-flows " + associatedBridge +
        " \"table=0, priority=50, ip_dst=" + expectedCIDR + ", ct_state=+trk+new, tcp\"");
    expectedCmds.push_back("ovs-ofctl --strict del-flows " + associatedBridge +
        " \"table=0, priority=50, ip_dst=" + expectedCIDR + ", ct_state=-trk, udp\"");
    expectedCmds.push_back("ovs-ofctl --strict del-flows " + associatedBridge +
        " \"table=0, priority=50, ip_dst=" + expectedCIDR + ", ct_state=+trk+new, udp\"");
    expectedCmds.push_back("ovs-ofctl add-flow " + associatedBridge +
        " \"table=0, priority=50, ip_dst=" + expectedCIDR +
        ", ct_state=-trk, tcp, actions=ct(table=0)\"");
    expectedCmds.push_back("ovs-ofctl add-flow " + associatedBridge +
        " \"table=0, priority=50, ip_dst=" + expectedCIDR +
        ", ct_state=+trk+new, tcp, actions=ct(commit),NORMAL\"");
    expectedCmds.push_back("ovs-ofctl add-flow " + associatedBridge +
        " \"table=0, priority=50, ip_dst=" + expectedCIDR +
        ", ct_state=-trk, udp, actions=ct(table=0)\"");
    expectedCmds.push_back("ovs-ofctl add-flow " + associatedBridge +
        " \"table=0, priority=50, ip_dst=" + expectedCIDR +
        ", ct_state=+trk+new, udp, actions=ct(commit),NORMAL\"");

    Gateway_Config cfg = {0};
    cfg.if_type = ifType;
    cfg.if_cmd = ifCmd;
    strcpy(cfg.if_name, const_cast<char *>(bridge.c_str()));
    strcpy(cfg.parent_bridge, const_cast<char *>(bridge.c_str()));
    cfg.mtu = atoi(mtu.c_str());
